...
# Backend se API key uthana (User ko nahi dikhega)
try:
    api_key = st.secrets["GEMINI_API_KEY"]
    client = genai.Client(api_key=api_key)
except KeyError:
    st.error("System Error: API Key not configured in the backend.")
    client = None
...
if uploaded_file is not None:
    ...
    if "Daily_Energy_kWh" in df.columns:
        ...
        rate = st.number_input("Electricity Rate (₹ per kWh)", value=8.0)
